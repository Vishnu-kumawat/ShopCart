...
import React, { useState } from 'react';
import { Menu, ChevronDown , Phone, Search, ShoppingCart, User } from 'lucide-react';
import { Link, useNavigate } from 'react-router-dom';
import useCartStore from '../store/AddToCart';
...
const Header = ({ openLogin, openSignUp }) => {
    ...
    return (
        <div>
            <div className="bg-green-900 text-white text-xs sm:text-sm font-semibold py-2 w-full">
                <div className="container mx-auto flex flex-col md:flex-row justify-between items-center px-4 md:px-6 lg:px-10 space-y-2 md:space-y-0">
                    <div className="flex justify-center md:justify-start items-center space-x-2">
                        <Phone className="w-4 h-4 md:w-5 md:h-5" />
                        <p className="text-xs md:text-sm">+001234567890</p>
            ...
            <div className='w-full h-auto md:h-24 flex flex-col md:flex-row justify-between items-center px-4 sm:px-6 lg:px-10 space-y-4 md:space-y-0'>
                {/* Logo Section */}
                <div className='flex items-center justify-between w-full md:w-auto'>
                    <div className='flex items-center'>
                        <img
                            className='h-12 lg:-mt-4 md:-mt-7 sm:h-16 md:h-16 lg:h-16 xl:h-20 mix-blend-multiply'
                            src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS5BnamwQSHMGF3vutVgWOYYT-StE1VSmIf1Efk8ekAysodL7fr8ueSriNBzko9vhqSMPk&usqp=CAU"
                            alt="ShopCart Logo"
                        />
                        <p className='ml-2 text-md sm:text-lg md:text-xl lg:text-2xl xl:text-3xl font-semibold'>ShopCart</p>
                    </div>
                    {/* Mobile Menu Icon */}
                    <div className='flex md:hidden items-center space-x-1 cursor-pointer' onClick={() => setMenuOpen(!menuOpen)}>
                ...
                {/* Navigation Links */}
                <div className={`flex-col md:flex-row items-center md:flex ${menuOpen ? 'flex' : 'hidden'} space-y-2 md:space-y-0 space-x-0 md:space-x-6`}>
                    <div className='flex items-center space-x-1'>
                        <p className='text-sm sm:text-base md:text-base lg:text-lg'>Category</p>
                    ...
                    <p className='text-sm sm:text-base md:text-base lg:text-lg'>What's New</p>
                    <p className='text-sm sm:text-base md:text-base lg:text-lg'>Delivery</p>
                    {/* Account and Cart Section for Mobile */}
                ...
                {/* Search Bar */}
                <form className='hidden lg:flex items-center relative mb-2 lg:mb-0' onSubmit={handleSearch}>
                    <input
                        className='border-2 rounded-xl w-24 md:w-32 lg:w-48 xl:w-64 border-gray-400 px-3 py-1 text-sm lg:text-base xl:text-lg'
                        type="text"
                        placeholder='Search Product'
                        value={searchInput}
                        onChange={(e) => setSearchInput(e.target.value)}
                ...
                {/* Account and Cart Section for Larger Devices */}
